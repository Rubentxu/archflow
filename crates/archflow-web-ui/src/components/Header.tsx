...
export default function Header({ className }: HeaderProps) {
  ...
  const { bridge, isInitialized: isWasmInitialized } = useArchFlowWasm();
  const backend = useBackend(bridge, null, isWasmInitialized);
    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="hidden lg:block mr-2">
          <BackendSelector
            backendInfo={backend.availableBackends}
            selectedBackend={backend.selectedBackend}
            onBackendChange={backend.selectBackend}
            isInitializing={!backend.isGraphicsReady && isWasmInitialized}
            error={backend.graphicsError}
          />
        </div>
        ...
        <div className="flex -space-x-2">
          <div className="relative inline-block">
            <div
              className="w-8 h-8 rounded-full border-2 border-white dark:border-surface-dark bg-cover bg-center bg-gray-300"
              style={{
                backgroundImage:
          ...
          <div className="relative inline-block">
            <div
              className="w-8 h-8 rounded-full border-2 border-white dark:border-surface-dark bg-cover bg-center bg-gray-400"
              style={{
                backgroundImage:
